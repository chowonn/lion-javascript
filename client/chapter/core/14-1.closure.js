...
// 클로저 (closure) 란?
// 전역의 오염을 막기 위해, 가비지 컬렉터가 수집을 할 수 없게 => 값을 기억하기 위해
// 내가 설정한 값을 수정할수 없게 끔(특정 함수를 통해서만 접근)
...
const value = first() // 가비지 컬렉터가 가져가지 않음. 세컨드 함수를 내보냈기 때문에 세컨드 함수가 태어난 환경을 기억하기 때문에 가비지 컬렉터가 안가져가는것.
 



function counter(){ //result () => ++count 와 같음 리턴없이
  let count = 0; 
  ...
  return incre; //incre () 처럼 실행하는게 아닌 incre 로 그냥 그 자체를 내보내는 것
}

let result = counter(); //counter() 이 할당된거고 그 환경이 살아있고 result 변수가 가지고 있으므로 가비지컬렉터의 수집 대상에서 제외임.
